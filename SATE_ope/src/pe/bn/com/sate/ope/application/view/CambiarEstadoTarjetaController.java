...
public class CambiarEstadoTarjetaController implements Serializable {
	...
	public void cambiarEstadoTarjeta() {
		...
		try {
			if (cambiarEstadoTarjetaModel.getTipoBloqueoSeleccionado().equals(
					"T")) {
				cambiarEstadoTarjetaModel.iniciarEstadoTarjeta();
				// try {
				// ModificacionTarjeta modificacionTarjeta = fwmcProcesos
				// .modificarTarjeta(
				...
				// .equals("0000")) {
				System.out.println(cambiarEstadoTarjetaModel.getEstadoTarjeta()
						.toString());
				if (cambiarEstadoTarjetaModel.getEstadoTarjeta().getMotivo()
						.equals(MotivosBloqueoTarjeta.ROBO.getId())) {
					UsefulWebApplication.ejecutar("dgSolicitarTarjeta.show()");
				} else {
					tarjetaService
							.actualizarEstadoTarjeta(cambiarEstadoTarjetaModel
									.getEstadoTarjeta());
				...
				cambiarEstadoTarjetaModel
						...
						.setEstadoCuenta(
								cambiarEstadoTarjetaModel
										.getEstadoCuentaSeleccionado());

				tarjetaService.actualizarEstadoCuenta(cambiarEstadoTarjetaModel
						.getDatosTarjetaCliente().getTarjeta());
				cambiarEstadoTarjetaModel.inicializarFormulario();
				UsefulWebApplication.mostrarMensajeJSF(
						ConstantesGenerales.SEVERITY_INFO, "",
